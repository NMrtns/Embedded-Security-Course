...
int increase_counter(const char* username) {
    printf("Increasing counter for user: %s\n", username);
    FILE* file = fopen(FILE_USERS, "r+");
    if (file == NULL) {
        printf("Could not open hashed_users.txt\n");
        return -1;
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // Split the line into username and counter
        char* token = strtok(line, ":");
        if (token != NULL) {
            strcpy(file_username, token);
            ...
            if (token != NULL) {
                ...
                if (token != NULL) {
                    ...
                    if (token != NULL) {
                        ...
                        if (token != NULL) {
                            strcpy(file_counter, token);
                            printf("counter before increment: %s\n", file_counter);
                        }
                    }
                }
            }
...
int main() {
    ...
    if (check_login(username, password)) {
        ...
        if (counter > 0) {
            ...
            printf("Failed to increase counter.\n");
        }

        if (counter > 3 || counter_reached) {
